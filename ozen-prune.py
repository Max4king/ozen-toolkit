...
if __name__ == '__main__':
    ...
    if os.path.isfile(file_path):
        ...
        if file_ext in ['.mp3', '.m4a', '.flac', '.ogg', '.wav']:
            ...
            if args.mode == 'segment and transcribe':
                ...
                pb = tqdm(total=len(groups))
                print("There are " +  str(amount_to_train) + " training files and " + str(amount_to_valid) + " validation files")
                print(colorama.Fore.GREEN + 'Transcribing for training...' + colorama.Fore.RESET)
                for wav in range(0, amount_to_train):
                    wav = str(wav)
                    # print(f"Transcribing training file: {wav}")
                    res = transcribe_audio(os.path.join(wavs_path, wav + '.wav'), device=args.device)
                    # print(f"Transcribe training file Complete: {wav}")
                    # add to train file
                    add_to_textfile(train_file, 'wavs/' + wav + '.wav|' + res + '\n')
                    pb.update(1)
                print(colorama.Fore.GREEN + 'Transcribing for validation...' + colorama.Fore.RESET)
                for wav in range(amount_to_train, amount_to_train + amount_to_valid):
                    # print(f"Transcribing validation file: {wav}")
                    wav = str(wav)
                    res = transcribe_audio(os.path.join(wavs_path, wav + '.wav'), device=args.device)
                    # add to valid file
    ...
    #                     gidx += 1
    #                     wav = str(wav)
    #                     res = transcribe_audio(os.path.join(wavs_path, str(gidx)+'.wav'))
    #                     # add to train file
